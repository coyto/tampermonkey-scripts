    ...
    config: {
      ...
      post: {
        ...
        createSkipDownloadCheckbox: (postId, checked) => {
          return ui.forms.createCheckbox(
            `settings-${postId}-skip-download`,
            'Skip Download',
            checked
          );
        },
        ...
        createGenerateLinksCheckbox: (postId, checked) => {
          return ui.forms.createCheckbox(
            `settings-${postId}-generate-links`,
            'Generate Links',
            checked
          );
        },
        ...
        createSkipDuplicatesCheckbox: (postId, checked) => {
          return ui.forms.createCheckbox(
            `settings-${postId}-skip-duplicates`,
            'Skip Duplicates',
            checked
          );
        },
        ...
        createToggleAllCheckbox: postId => {
          return ui.forms.createCheckbox(
            `settings-toggle-all-hosts-${postId}`,
        ...
        createHostCheckbox: (postId, host) => {
          ...
          return ui.forms.createCheckbox(
            `downloader-host-${host.id}-${postId}`,
            `${title} (${host.resources.length})`,
            host.enabled
          );
        },
        /**
        ...
        createPostConfigForm: (
          ...
          let hostsHtml = '<div>';
          parsedHosts.forEach(
            host => (hostsHtml += ui.forms.config.post.createHostCheckbox(postId, host))
          );
          hostsHtml += '</div>';

          const filterLabel = ui.forms.config.post.createFilterLabel(
            parsedHosts,
            totalDownloadableResourcesForPostCB
          );
          ...
          let formHtml = [
            window.isFF
              ? ui.forms.config.post.createFilenameInput(
                  customFilename,
                  postId,
                  color,
                  defaultFilename
            ...
            ui.forms.config.post.createHostCheckboxes(
              postId,
              filterLabel,
              hostsHtml,
              parsedHosts.length > 1
            ),
            ui.forms.createRow(
          ...
          ui.tooltip(btnDownloadPost, configForm, {
            onShown: instance => {
              ...
              h.element(`#settings-${postId}-skip-download`).addEventListener('change', e => {
                ...
                h.element(`#settings-${postId}-flatten`).checked = checked
                  ? false
                  : prevSettings.flatten;
                h.element(`#settings-${postId}-flatten`).disabled = checked;

                h.element(`#settings-${postId}-skip-duplicates`).checked = checked
                  ? false
                  : prevSettings.skipDuplicates;
                h.element(`#settings-${postId}-skip-duplicates`).disabled = checked;

                h.element(`#settings-${postId}-generate-links`).checked = checked
                  ? true
                  : prevSettings.generateLinks;
              ...
              if (parsedHosts.length > 1) {
                h.element(`#settings-toggle-all-hosts-${postId}`).addEventListener(
                  'change',
                  async e => {
                    e.preventDefault();
                    ...
                    const hostCheckboxes = parsedHosts.flatMap(host =>
                      h.element(`#downloader-host-${host.id}-${postId}`)
                    );
              ...
              parsedHosts.forEach(host => {
                h.element(`#downloader-host-${host.id}-${postId}`).addEventListener('change', e => {
                  ...
                  if (parsedHosts.length > 0) {
                    ...
                    const totalResources = parsedHosts.reduce(
                      (acc, host) => acc + host.resources.length,
                      0
                    );
                    ...
                    if (parsedHosts.length > 1) {
                      const toggleAllHostsCheckbox = h.element(
                        `#settings-toggle-all-hosts-${postId}`
                      );
