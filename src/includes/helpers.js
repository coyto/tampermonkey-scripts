const h = {
   ...
   * @returns {string|*}
   */
  limit: (string, maxLength = 20) =>
    string.length > maxLength ? `${string.substring(0, maxLength - 1)}...` : string,
  /**
   * @param selector
   * @param container
   * @returns {*}
  ...
  contains: (needle, haystack, ignoreCase = true) =>
    (ignoreCase ? haystack.toLowerCase().indexOf(needle.toLowerCase()) : haystack.indexOf(needle)) >
    -1,
  /**
   * @param str
   * @returns {*|string}
  ...
  unique: (items, cb) => {
    if (cb) {
      return items.reduce(
        (acc, item) => (!acc.find(i => i[byKey] === item[byKey]) ? acc.concat(item) : acc),
        []
      );
    }
  ...
  http: {
    ...
    get: (url, callbacks = {}, headers = {}, responseType = 'document') => {
      return h.promise(resolve =>
        resolve(h.http.base('GET', url, callbacks, headers, {}, responseType))
      );
    },
    /**
     * @param url
