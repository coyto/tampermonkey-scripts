const parsers = {
  thread: {
    ...
    parseTitle: () => {
      ...
      let parsed = h
        .stripTags(['a', 'span'], h.element('.p-title-value').innerHTML)
        .replace('/\n/g', '');
     ...
     */
    parsePost: post => {
      const messageContent = post.parentNode.parentNode.querySelector(
        '.message-content > .message-userContent > .message-body'
      );
      ...
      [
        ...messageContentClone.querySelectorAll(
          '.contentRow-header > a[href^="https://simpcity.su/threads"]'
        ),
      ]
      ...
      const spoilers = [
        ...messageContentClone.querySelectorAll('.bbCodeBlock--spoiler > .bbCodeBlock-content'),
      ]
        .filter(s => !s.querySelector('.bbCodeBlock--unfurl'))
        .concat(
          [...messageContentClone.querySelectorAll('.bbCodeInlineSpoiler')].filter(
            s => !s.querySelector('.bbCodeBlock--unfurl')
          )
        )
        .map(s => s.innerText)
        .concat(
      ...
      for (const host of hosts) {
        ...
        const execMatcher = matcher => {
          ...
          if (h.contains('!!', pattern)) {
            ...
            const pat = `(?<=data-url="|src="|href=")https?:\/\/(www.)?${h.re.toString(
              pattern
            )}.*?(?=")|https?:\/\/(www.)?${h.re.toString(pattern)}.*?(?=("|<|$|\]|'))`;
