 ...
 */
const hosts = [
  ['simpcity.cr:Attachments', [/simpcity.cr\/attachments/]],
  ...
  [
    'jpg.fish:image',
    [
      /(simp\d+.)?jpg.(church|fish|fishing)\/(?!(images\/0fya082315al\.png|img\/))/,
      /jpg.(church|fish|fishing)\/a\/[~an@-_.]+<no_qs>/,
    ],
  ],
  ...
  [
    'instagram.com:Media',
    [/!!https:(\/|\\\/){2}s9e.github.io(\/|\\\/)iframe(\/|\\\/)2(\/|\\\/)instagram.*?(?="|&quot;)/],
  ],
  [
    'instagram.com:Profile',
    [/!!instagram.com\/[~an@_.-]+|((instagram|insta):(\s+)?)@?[a-zA-Z0-9_.-]+/],
  ],
  ...
  [
    'cyberfile.me:',
    [/!!https:\/\/cyberfile.(su|me)\/\w+(\/)?(?=")/, /cyberfile.(su|me)\/folder\//],
  ],
...
const resolvers = [
  ...
  [
    ...
    async (url, http) => {
      ...
      for (const post of posts) {
        ...
        const attachments = dom.querySelectorAll(
          '.post__attachments > .post__attachment > .post__attachment-link'
        );
  ...
  [
    ...
    async (url, http) => {
      url = url.replace(
        /https?:\/\/(www.)?i\.?(postimg|pixxxels).cc\/(.{8})(.*)/,
        'https://postimg.cc/$3'
      );
      const { dom } = await http.get(url);
  ...
  [
    ...
    async (url, http, spoilers, postId) => {
      ...
      if (h.contains('Please enter your password to continue', source)) {
        ...
        const authToken = authTokenNode ? authTokenNode.getAttribute('value') : null;
        ...
        if (!authenticated) {
          log.host.error(
            postId,
            `::Could not resolve password protected album::: ${url}`,
            'jpg.fishing'
          );
          return null;
        }
  ...
  [
    [/([a-z](\d+)?\.)pixl.(li|is)\/((img|image)\/)?/, /:!pixl.(li|is)\/album\//],
  ...
  [
    ...
    async (url, http, spoilers, postId) => {
      ...
      let { source, dom } = await http.get(currentUrl, {
        onStateChange: response => {
          // If it's a redirect, we'll have to fetch the new url.
          if (response.readyState === 2 && response.finalUrl !== currentUrl) {
      ...
      if (reFetch) {
        const { source: src, dom: d } = await http.get(currentUrl);
        source = src;
        dom = d;
      }
  ...
  [
    [/t(\d+)?\.pixhost.to\//, /:!pixhost.to\/gallery\//],
  ...
  [
    ...
    async (url, http) => {
      ...
      const resolved = h.re
        .matchAll(
          /(?<=\[img])https:\/\/t\d+.*?(?=\[\/img])/gis,
          imageLinksInput.getAttribute('value')
        )
  ...
  [
    [
      /((stream|cdn(\d+)?)\.)?bunkr.(ru|su|la).*?\.[a-zA-Z0-9]{3,4}|i(\d+)?.bunkr.(ru|su|la)\/(v\/)?/i,
      /:!bunkr.(ru|su|la)\/a\//,
    ],
    async (url, http) => {
      const cleanUrl = url.replace('stream.bunkr', 'bunkr').replace(/cdn(\d+)?\.bunkr/, 'bunkr');

      const ext = h.ext(cleanUrl).toLowerCase();

      let processedUrl = cleanUrl;
      if (settings.extensions.video.includes(`.${ext}`) && !h.contains('/v/', cleanUrl)) {
        processedUrl = cleanUrl.replace(/(bunkr.(ru|su|la))\//, '$1/v/');
      }

      if (settings.extensions.image.includes(`.${ext}`)) {
        return cleanUrl.replace(/cdn(\d+)?/, 'i$1');
      }

      let finalUrl = processedUrl;
      if (['zip', 'pdf'].includes(ext) && !h.contains('/d/', processedUrl)) {
        finalUrl = processedUrl.replace(/(bunkr.(ru|su|la))\//, '$1/d/');
      }
      ...
      return btnDownload ? btnDownload.getAttribute('href') : null;
    },
  ],
  [
    [/bunkr.(ru|su|la)\/a\//],
    ...
    async (url, http) => {
      ...
      while (true) {
        ...
        if (h.contains('_v', source)) {
          ...
          } else if (firstMediaId === parsed[0].id) {
            break;
          }
          resolved.push(
            ...parsed.flatMap(i => {
  ...
  [
    ...
    async (url, http) => {
      ...
      return `https://${subDomain}.zippyshare.com${expr}`;
    },
  ],
  [
    [/pixeldrain.com\/[ul]/],
  ...
  [
    ...
    async (url, http) => {
      const cleanUrl = url.replace(/([a-zA-Z0-9]+\.)?pornhub/, 'pornhub');
      ...
      // TODO: Fix me
      do {
        const infoURL = await resolvePH(cleanUrl);
      ...
      const resolveAlbum = async (url, spoilers) => {
        ...
        if (h.contains('error-passwordRequired', source) && spoilers.length) {
          ...
          if (spoilers.length) {
            log.host.info(
              postId,
              `::Trying with ${spoilers.length} available password(s)::`,
              'gofile.io'
            );
          }
  ...
  [
    ...
    async (url, http, spoilers) => {
      ...
      if (
        (h.contains('albumPasswordModel', response) ||
          h.contains('This folder requires a password', response)) &&
        spoilers.length
      ) {
  ...
  [
    ...
    async (url, http, spoilers) => {
      ...
      const script = [...dom.querySelectorAll('script')]
        .map(s => s.innerText)
        .filter(s => h.contains('data-toggle="tab"', s))[0];
      ...
      if (props && props.html) {
        ...
        for (const fileUrl of urls) {
          ...
          resolved.push(
            h.re.matchAll(/(?<=openUrl\(').*?(?=')/gi, response)[0]?.replace(/\\\//gi, '/')
          );
        }
      }
  ...
  [
    ...
    async (url, http) => {
      ...
      if (h.contains('urls', source)) {
        const { urls } = JSON.parse(source).gif;
        if (urls.hd) {
          return urls.hd;
        }
  ...
  [
    [/fs-\d+.cyberdrop.(me|to|cc|nl)\//, /:!cyberdrop.(me|to|cc|nl)\/a\//],
    url => url.replace(/(fs|img)-\d+/i, 'fs-01'),
  ],
  [
    [/cyberdrop.me\/a\//],
    async (url, http) => {
      ...
      let resolved = [...dom?.querySelectorAll('#file')].map(file =>
        file.getAttribute('href').replace(/fs-\d+/is, 'fs-01')
      );
  ...
  [
    ...
    async (url, http) => {
      ...
      let streamData = h.re
        .matchAll(/(?<=stream_data\s=\s){.*?}.*?(?=;)/gis, source)[0]
        .replace(/'/g, '"');
  ...
  [
    ...
    async (url, http) => {
      ...
      if (h.contains('gallery-name', source)) {
        ...
        const imageLinksInput = dom.querySelector(
          '.links.gallery > div:nth-child(2) > div > input'
        );

        const rawImageLinks = h.re.matchAll(
          /(?<=\[URL=).*?(?=])/gis,
          imageLinksInput.getAttribute('value')
        );
  ...
  [
    [/img.kiwi\/images\//, /:!img.kiwi\/album\//],
    url => url.replace('.th.', '.').replace('.md.', '.'),
  ],
  [
    [/img.kiwi\/album\//],
  ...
  [
    [/imgvb.com\/images\//, /:!imgvb.com\/album\//],
    url => url.replace('.th.', '.').replace('.md.', '.'),
  ],
  [
    [/imgvb.com\/album\//],
  ...
  [
    [/gfycat.com(\/|\\\/)/],
    async (url, http) => {
      const gfycatUrl = `https://gfycat.com/${url
        .replace(/&amp;/g, '&')
        .split('/')
        .reverse()[0]
        .replace(/\?.*/is, '')}?hd=1`;
      const { dom } = await http.get(gfycatUrl);
      return [...dom.querySelectorAll('source')]
        .map(el => el.getAttribute('src'))
        .filter(src => src && h.contains('giant.gfycat', src))[0];
    },
  ],
  [
    [/m\.box\.com\//],
    async (url, http) => {
      const { source, dom } = await http.get(url);
      const files = [...dom.querySelectorAll('.files-item-anchor')].map(
        el => `https://m.box.com${el.getAttribute('href')}`
      );
  ...
  [
    ...
    async (url, http) => {
      ...
      if (h.contains('s9e.github.io', cleanUrl)) {
        id = h.re.matchAll(/(?<=#).*/gis, cleanUrl)[0];
        if (id[0] === 'a') {
          type = 'album';
          id = id.substring(2);
        }
      } else {
        if (h.contains(cleanUrl, 'a/')) {
          type = 'album';
        }
        id = cleanUrl.split('/').reverse()[0];
      }
  ...
  [
    ...
    url =>
      url.replace(
        /https?:\/\/pbs.twimg\.com\/media\/(.{1,15})(\?format=)?(.*)&amp;name=(.*)/,
        'https://pbs.twimg.com/media/$1.$3'
      ),
  ],
  [
    [/(disk\.)?yandex\.[a-z]+/],
  ...
  [
    ...
    async (url, http) => {
      ...
      const script = [...dom.querySelectorAll('script')]
        .map(s => s.innerText)
        .filter(s => h.contains('shortcode_media', s))[0];

      const resolved = [];

      if (script && h.contains('"is_video":true', script)) {
  ...
  [
    ...
    async (url, http) => {
      ...
      const { source: response } = await h.http.get(
        `${apiBaseUrl}${profileId}/?count=100`,
        {},
        headers
      );
      ...
      const collect = async response => {
        ...
        if (props && props.status === 'ok' && props.num_results > 0) {
          ...
          if (props.more_available === true && props.next_max_id) {
            ...
            const { source } = await h.http.get(
              `${apiBaseUrl}/${profileId}/?count=100&max_id=${props.next_max_id}`,
              {},
              headers
            );
...
const downloadPost = async (
  ...
  log.post.info(
    postId,
    `::Using ${enabledHosts.length} host(s)::: ${enabledHosts.map(h => h.name).join(', ')}`,
    postNumber
  );
        ...
        const request = GM_xmlhttpRequest({
          ...
          onreadystatechange: response => {
            if (response.readyState === 2) {
              let matches = h.re.matchAll(
                /(?<=attachment;filename=").*?(?=")/gis,
          ...
          onprogress: response => {
            ...
            if (response.total === -1 || response.totalSize === -1) {
              ...
              h.ui.setText(
                statusLabel,
                `${completed} / ${totalDownloadable} 🢒 ${host.name} 🢒 ${downloadedSizeInMB} MB 🢒 ${ellipsedUrl}`
              );
            } else {
              h.show(filePB);
              h.ui.setText(
          ...
          onload: response => {
            ...
            if (url.includes('https://pixeldrain.com/')) {
              basename = response.responseHeaders
                .match(/^content-disposition.+filename=(.+)$/im)[1]
                .replace(/"/g, '');
            } else if (url.includes('https://simpcity.su/attachments/')) {
              basename = filename
                ? filename.name
                : h.basename(url).replace(/(.*)-(.{3,4})\.\d*$/i, '$1.$2');
              ...
              basename = filename
                ? filename.name
                : h.basename(url).replace(/\?.*/, '').replace(/#.*/, '');
            }
  ...
  if (totalDownloadable > 0) {
    ...
    if (!isFF && !postSettings.zipped && (postSettings.generateLog || postSettings.generateLinks)) {
...
const registerPostReaction = postFooter => {
  ...
  if (!hasReaction) {
    ...
    if (reactionAnchor) {
      reactionAnchor.setAttribute(
        'href',
        reactionAnchor.getAttribute('href').replace('_id=1', '_id=33')
      );
...
(function () {
  window.addEventListener('beforeunload', e => {
    if (processing.find(p => p.processing)) {
      return 'Downloads are in progress. Sure you wanna exit this page?';
    }
  });

  document.addEventListener('DOMContentLoaded', async () => {
    const goFileTokenFetchFailedErr =
      'Failed to create GoFile token. GoFile albums may not work. Refresh the browser to retry.';
    ...
    if (parsedPosts.filter(p => p.parsedHosts.length).length > 0) {
      ...
      let html = ui.forms.createCheckbox(
        'config-toggle-all-posts',
        settings.ui.checkboxes.toggleAllCheckboxLabel,
        false
      );
      ...
      parsedPosts
        ...
        .forEach(post => {
          ...
          const ellipsedText = h.limit(
            defaultPostContent === '' ? threadTitle : defaultPostContent,
            20
          );

          const summary = `<a id="post-content-${postId}" href="#post-${postId}" style="color: dodgerblue"> ${ellipsedText} </a>`;
          html += ui.forms.createCheckbox(
            `config-download-post-${postId}`,
            `Post #${postNumber} ${summary}`,
            false
          );
        });
        ...
        onShown: () => {
          parsedPosts
            ...
            .forEach(post => {
              ...
              document
                .querySelector(`#config-download-post-${postId}`)
                .addEventListener('change', e => {
                  ...
                  checkAllCB.checked =
                    selectedPosts.filter(s => s.enabled).length === parsedPosts.length;
                });
